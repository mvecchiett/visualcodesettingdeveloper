...
def Alta():
    if  NOMBRE.get() == "" or APELLIDO.get() == "" or SEXO.get() == "" or EMAIL.get() == "" or USERNAME.get() == "" or PASSWORD.get() == "":
        ...
        cursor.close()
        conn.close()
        txt_result.config(text="Usuario creado con éxito,", fg="green")
...
def Validardatos():
    ...
    if SEXO.get() == "":
        if re.match('^[(a-z0-9\_\-\.)]+@[(a-z0-9\_\-\.)]+\.[(a-z)]{2,15}$',EMAIL.lower()):
            ...
            txt_result.config(text="Direccion de email incorrecto.", fg="red")
    else:
        txt_result.config(text="Por favor, seleccione el sexo.", fg="red")
...
#    if SEXO.get() == "":
    if Validardatos()==FALSE:
        ...
        txt_result.config(text="Usuario modificado con éxito", fg="black")
...
def Borrar():
    if not tree.selection():
       txt_result.config(text="Por favor, seleccione un registro primero,", fg="red")
    else:
        result = tkMessageBox.askquestion("Gestor de Usuarios", "Esta seguro de eliminar este registro?", icon="warning")
        if result == 'yes':
            curItem = tree.focus()
            contents =(tree.item(curItem))
            selecteditem = contents['values']
...
#==================================ETIQUETAS==========================================
txt_title = Label(Frametitulo, width=900, font=('arial', 24), text = mensajes[0])
txt_title.pack()
